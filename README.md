...
נוסף ממשק React בעברית עם API בפייתון: התאמת יעד, מסלול לפי תאריכים, הצגת שעות,
ייצוא לטקסט ול־JSON, בדיקות עומס וחפיפות, ביקורת מודל ורשימת אריזה לפי תחזית.
להוראות הפעלה ומגבלות האימות ראו [frontend/README.md](frontend/README.md).
אחרי התקנת התלויות ובניית הממשק אפשר להפעיל ב־Windows באמצעות `run_web.bat`.
נתוני נסיעה ושעות פתיחה ממקור חי עדיין אינם מחוברים; הם מסומנים כחסרים בבדיקת המסלול.
...
| Trip Planning Agent (`agent/trip_planner.py`) | [Gemini API](https://ai.google.dev) (`gemini-3.5-flash-lite`, גיבוי `gemini-3.6-flash`) | דורש `GEMINI_API_KEY` משלכם. זמינות ומכסות תלויות במודל ובחשבון. מודל חילוץ הפרופיל מוגדר בנפרד ב־`nlp/`. |
